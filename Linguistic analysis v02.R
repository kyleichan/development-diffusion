...
# Choose ONE of the following versions of the Maddison growth pattern data:
mad <- read.csv("./input_data/maddison_subset5.csv") # 1950-2008, 137 countries, 5-year CAGR ----- PRIMARY OPTION
...
mad <- read.csv("./input_data/maddison_subset_africa.csv") # 1950-2008, 48 countries, 5-year CAGR, Sub-Saharan Africa only
mad <- read.csv("./input_data/maddison_subset_eastasia.csv") # 1950-2008, 15 countries, 5-year CAGR, East Asia only
mad <- read.csv("./input_data/maddison_subset_opec.csv") # 1950-2008, 13 countries, 5-year CAGR, OPEC members only

mad <- read.csv("./input_data/maddison_subset_exeurope.csv") # 1950-2008, 115 countries, 5-year CAGR, excluding Europe
mad <- read.csv("./input_data/maddison_subset_exanglo.csv") # 1950-2008, 131 countries, 5-year CAGR, excluding Anglophone countries (USA, UK, Australia, Ireland, Canada)
mad <- read.csv("./input_data/maddison_subset_exopec.csv") # 1950-2008, 124 countries, 5-year CAGR, excluding OPEC members
...
# Create distance matrix for 2008 levels of income
mad08_2 <- as.matrix(log(mad08$X2008)) # Extract the income numbers
rownames(mad08_2) <- mad08$country_code # Rename rows using country codes
mad08.dist.matrix <- vegdist(mad08_2, "euclidean")
...
ecodist::mantel(growth.dist.matrix ~ lang.dist.matrix, nperm = 100000)
ecodist::mantel(mad08.dist.matrix ~ lang.dist.matrix, nperm = 100000)

# Extra Mantel tests
ecodist::mantel(growth.dist.matrix ~ lang.dist.matrix, nperm = 99999)
ecodist::mantel(growth.dist.matrix ~ lang.dist.matrix + geo.dist.matrix, nperm = 100000)

ecodist::mantel(growth.dist.matrix ~ geo.dist.matrix, nperm = 100000)
ecodist::mantel(growth.dist.matrix ~ geo.dist.matrix + lang.dist.matrix, nperm = 100000)

ecodist::mantel(growth.dist.matrix ~ lang.dist.matrix + mad08.dist.matrix, nperm = 100000)
ecodist::mantel(growth.dist.matrix ~ lang.dist.matrix + geo.dist.matrix + mad08.dist.matrix, nperm = 100000)


ecodist::mantel(mad08.dist.matrix ~ lang.dist.matrix, nperm = 100000)
ecodist::mantel(mad08.dist.matrix ~ lang.dist.matrix + geo.dist.matrix, nperm = 100000)
ecodist::mantel(mad08.dist.matrix ~ geo.dist.matrix + lang.dist.matrix, nperm = 100000)
ecodist::mantel(mad08.dist.matrix ~ geo.dist.matrix, nperm = 100000)
...
df2 <- arrange(df, value)


#######################
# Scrap code for trying to convert distance matrix into matrix with binary pairs and their distance values
# Used melt() function in reshape2 package instead

n <- 20
m <- growth.dist.matrix2
for(i in 1:n) {
  country1 <- rownames(m)[which(m == min(m, na.rm=TRUE), arr.ind = TRUE)[1]]
  country2 <- colnames(m)[which(m == min(m, na.rm=TRUE), arr.ind = TRUE)[2]]
  dist <- round(min(m, na.rm=TRUE), digits = 4)
  print(c(country1, country2, dist))
  m[which(m == min(m, na.rm=TRUE), arr.ind = TRUE)] <- NA
}


dist.matrix <- growth.dist.matrix
dist.matrix <- as.matrix(dist.matrix)
n <- nrow(dist.matrix)
m <- as.data.frame(matrix(nrow = (n*(n-1))/2, ncol = 3))
colnames(m) <- c("country1", "country2", "dist")

for (i in 1:n) {
  for (j in i:n) {
    if(i != j) {
      m$country1[i+j-2] <- rownames(dist.matrix)[i]
      m$country2[i+j-2] <- colnames(dist.matrix)[j]
      m$dist[i+j-2] <- dist.matrix[i, j]
      print(i+j-2)
    }
    else return
  }
...
plot(mgram(mad08.dist.matrix, geo.dist.matrix, nclass = 10))
plot(mgram(growth.dist.matrix, geo.dist.matrix, nclass = 10))
plot(mgram(growth.dist.matrix, lang.dist.matrix, nclass = 2))
plot(mgram(mad08.dist.matrix, lang.dist.matrix, nclass = 10))

mgram(growth.dist.matrix, lang.dist.matrix, nclass = 3)
plot(mgram(growth.dist.matrix, lang.dist.matrix, nclass = 4))
...
ecodist::mantel(growth.dist.matrix ~ geo.dist.matrix, nperm = 99999)
ecodist::mantel(growth.dist.matrix ~ geo.dist.matrix + lang.dist.matrix, nperm = 99999)
