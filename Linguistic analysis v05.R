...
##############################################################################################################
# Create trade data
# (run if you need to create the trade_distance_matrix.csv file)
##############################################################################################################
...
#################################################################
# Functions for running a set of Mantel tests
#################################################################
...
format.results <- function(results) {
  cell1 <- paste0(formatC(round(results[[1]][1], digits = 3), digits = 3, format = "f"), " (",format(results[[1]][4], scientific=TRUE, digits = 2),")")
  cell2 <- paste0(formatC(round(results[[2]][1], digits = 3), digits = 3, format = "f"), " (",format(results[[2]][4], scientific=TRUE, digits = 2),")")
  cell3 <- paste0(formatC(round(results[[3]][1], digits = 3), digits = 3, format = "f"), " (",format(results[[3]][4], scientific=TRUE, digits = 2),")")
...
################################################################################################################################
# Trade analysis
# (cannot use ecodist due to NAs in trade distance matrix; instead use ncf package, which does pairwise complete observations)
################################################################################################################################

# Create distance matrices, this time also create one for trade
matrices.w.trade <- create.dist.matrices(mad_subset5, add.trade=TRUE)

# Find total number of non-missing distance values in the trade distance matrix
...
# Note: ncf needs arguments as matrices, not dist objects
...
# Use results from ncf partial mantel to manually create output table for LaTeX
...
stats.row <- function(data, data.name) {
  ...
  geographic <- format.stats(stats[[3]], 3)
  income <- format.stats(stats[[4]], 3)
...
colnames(full.table2) <- c("Countries", "Distance \\hspace{20 mm} Values", "(1) \\hspace{20 mm} Growth", "(2) \\hspace{20 mm} Linguistic", "(3) \\hspace{20 mm} Geographic", "(4) \\hspace{20 mm} Income")
print(xtable(full.table2, align = c("l", "r", "p{1.2cm}", rep("p{2.1cm}", 4))), type='latex', sanitize.text.function=identity)


##############################################################################################################
# Correlation matrix across distance measures
##############################################################################################################

format.mantel <- function(mantel.result) {
  paste0(formatC(round(mantel.result[1], digits = 3), digits = 3, format = "f"), " (",format(mantel.result[4], scientific=TRUE, digits = 2),")")
}

create.cor.matrix <- function(mad) {
  matrices <- create.dist.matrices(mad)
  cor.matrix <- data.frame(matrix(ncol = 4, nrow = 4))
  matrix.names <- c("Growth", "Linguistic", "Geographic", "Income")
  rownames(cor.matrix) <- matrix.names
  colnames(cor.matrix) <- matrix.names
  
  n <- 100000
  cor.matrix[2, 1] <- format.mantel(ecodist::mantel(matrices[[1]] ~ matrices[[2]], nperm = n))
  cor.matrix[3, 1] <- format.mantel(ecodist::mantel(matrices[[1]] ~ matrices[[3]], nperm = n))
  cor.matrix[4, 1] <- format.mantel(ecodist::mantel(matrices[[1]] ~ matrices[[4]], nperm = n))
  cor.matrix[3, 2] <- format.mantel(ecodist::mantel(matrices[[2]] ~ matrices[[3]], nperm = n))
  cor.matrix[4, 2] <- format.mantel(ecodist::mantel(matrices[[2]] ~ matrices[[4]], nperm = n))
  cor.matrix[4, 3] <- format.mantel(ecodist::mantel(matrices[[3]] ~ matrices[[4]], nperm = n))
  
  return(cor.matrix)
}

c.matrix <- create.cor.matrix(mad_subset5)
print(xtable(c.matrix))
