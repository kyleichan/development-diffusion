...
* Choosing the technology for which the program is run
local technology_no = 15
local noofcountries = 4

* Choosing the gamma specifications for which the program is run
local gammazero 1
local gammanonzero 1 

* Include the usercost term in the estimation
local include_uc_term 0
...
* Set the example technology
local example_tech = 15
capture local b3usa = 3.3137536
capture local b5usa = .02725478
...
* Loading the data
local usafile `rootpath'`logpath'DTA\ResultsUSA.dta
local datafile `rootpath'CHExplorationData.dta
use `datafile', clear

matrix input countries = ( 155 , 73 , 5 , 110 )
local noofcountries = 4
...
* --------------------------------------------------------------------------------------------------------------------------------------
forvalues country_index = 1(1)`noofcountries' {

local chosen_country_no = countries[1,`country_index']  
...
* Defining the sample selection criterion
...
local fitfilestr "FIT_"
local logfilename `fitfilestr'`country_name'`underscorestr'`techname'
...
local eqstring "( lnYtou = lnY + {b1} + (`mupar'-1)*ln(t-{b2=`b2start'})*`b3usa' - (1-`alfapar')*lnYpercap*`b3usa' + `b5usa'*t  ) "
...
capture noisily list t lnYtou lnYtouhat if country_no==`chosen_country_no' , separator(0)
capture log off
set trace off
